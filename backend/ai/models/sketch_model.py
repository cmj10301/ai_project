...
def load_quickdraw_dataset(data_dir, class_names, samples_per_class=2000):
    ...
    for class_name in class_names:
        ...
        drawings = np.load(path, mmap_mode='r')  # 메모리 절약
...
def create_model_mobilenetv2(num_classes):
    base_model = MobileNetV2(
        input_shape=(96, 96, 3),
        include_top=False,
        weights=None,  # 사전학습된 가중치 안 씀 (1채널이라 맞지 않음)
        pooling='avg'
    )
    model = models.Sequential([
        layers.Resizing(96, 96),         # 28x28 → 96x96 확대
        layers.Conv2D(3, 1, padding='same'),  # 1채널 → 3채널 (RGB 흉내)
        base_model,
...
def train():
    ...
    with open(class_list_path, 'w', encoding='utf-8') as f:
        for name in class_names:
            f.write(name + '\n')
    print(f"✅ Class names saved to: {class_list_path}")
